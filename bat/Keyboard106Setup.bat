...
reg add "%KEY%" /v "LayerDriver JPN"            /t REG_SZ /d "kbd106.dll" /f > nul
reg add "%KEY%" /v "OverrideKeyboardIdentifier" /t REG_SZ /d "PCAT_106KEY" /f > nul
reg add "%KEY%" /v "OverrideKeyboardType"       /t REG_DWORD /d 7 /f > nul
reg add "%KEY%" /v "OverrideKeyboardSubtype"    /t REG_DWORD /d 2 /f > nul
reg query "%KEY%"
